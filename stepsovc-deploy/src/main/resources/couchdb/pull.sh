IFS=";"
export IFS
ids=$(curl -X POST http://localhost:5984/commcarehq/_temp_view   -H "Content-Type: application/json" -d @view.json | jsawk 'appIds=""; for(i=0;i<this.rows.length;i++) appIds=appIds+this.rows[i].id+";"; return appIds;')
for id in $ids; do
